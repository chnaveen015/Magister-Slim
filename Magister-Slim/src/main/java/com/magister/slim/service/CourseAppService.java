...
public class CourseAppService {
	...
	public Course deleteCourse(String offeringLevelId, String courseId) {
		Course courseDetails = (courseInterface.findAll()).stream()
				.filter(course -> course.getCourseId().equals(courseId)).findFirst().get();
	...
	public Course updateCourseDetails(String offeringLevelId, Course course) {
		Course courseDetails = (courseInterface.findAll()).stream()
				.filter(SingleCourse -> SingleCourse.getCourseId().equals(course.getCourseId())).findFirst().get();
		if (courseDetails != null) {
			...
			// boolean status =
			// groupAppService.updateCourseReferenceDetails(groupId,course);
			return courseDetails;
		}
		return null;
	}
	...
	public boolean updateGroupReferences(Group groupDetails) {
		...
		List<GroupReference> groupReferences = new ArrayList<GroupReference>();
		Course courseDetails = (courseInterface.findAll()).stream()
				.filter(course -> course.getCourseId().equals(groupDetails.getCourseReference().getCourseId()))
				.findFirst().get();
		if (courseDetails != null) {
	...
	public boolean deleteOfferingLevelreference(String offeringLevelId) {
		OfferingLevel offeringLevel = (offeringLevelInterface.findAll()).stream()
				.filter(offeringLevelDetails -> offeringLevelDetails.getOfferingLevelId().equals(offeringLevelId))
				.findFirst().get();

//		OfferingLevel offeringLevel = offeringLevelInterface.findById(offeringLevelId).get();
		if (offeringLevel.getCourseReferences() != null) {
			List<CourseReference> courseReferences = (offeringLevel.getCourseReferences()).stream()
					.map(courseReference -> {
						courseReference.setActive(false);
						Course course = (courseInterface.findAll()).stream().filter(
								singleCourse -> singleCourse.getCourseId().equals(courseReference.getCourseId()))
								.findFirst().get();
		...
		if (course.getGroupReferences() != null) {
			List<GroupReference> groupReferences = (course.getGroupReferences()).stream().map(groupReference -> {
				...
				Group group = groupInterface.findAll().stream()
						.filter(oneGroup -> oneGroup.getGroupId().equals(groupReference.getGroupId())).findFirst()
						.get();
	...
	public boolean updateStudyguideReferences(StudyGuide studyGuide) {
		...
		if (courseInterface.findById(studyGuide.getCourseReference().getCourseId()).isPresent()) {
			...
			studyGuideReferences
					.add(new StudyGuideReference(studyGuide.getStudyGuideId(), studyGuide.getStudyGuideName(), true));
	...
	public boolean deleteStudyGuideReference(StudyGuide studyGuide) {
		...
		if (courseInterface.findById(studyGuide.getCourseReference().getCourseId()).isPresent()) {
			Course courseDetails = courseInterface.findById(studyGuide.getCourseReference().getCourseId()).get();
			List<StudyGuideReference> studyGuideReferences = courseDetails.getStudyGuideReferences().stream()
					.map(stuyguideReference -> {
